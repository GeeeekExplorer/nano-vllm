...
class CPUKVCacheManager:
    ...
    # ================================================================
    #  昇腾 910C 专用：适配 scatter+burst 传输模式
    # ================================================================

    def store_packed_blocks(
        self,
        layer_name: str,
        seq_id: int,
        ...
        """将 scatter 打包后的连续 KV 数据按 block 粒度存入 CPU 存储池。

        由 CPUOffloadConnector.ascend_swap_out_with_scatter_burst 在
        burst DMA 传输完成后调用。此时 k_packed/v_packed 已在 CPU pin_memory 上。

        Args:
            layer_name: 层名称
            seq_id:     序列 ID
            k_packed:   打包后的连续 K 数据（CPU 端），
                        shape = [len(block_ids) * block_size, num_kv_heads, head_dim]
            v_packed:   打包后的连续 V 数据（CPU 端），shape 同上
            block_ids:  对应的 NPU 端 block ID 列表

        Returns:
            bool: 存储是否成功
        """
        # --- 执行逻辑 ---
        # 1. 将 k_packed / v_packed 按 block_size 切分为 len(block_ids) 段
        # 2. 对每段：
        #    a. 调用 allocate_cpu_block() 获取 cpu_block_id
        #    b. 若分配失败，回滚并返回 False
        #    c. 将该段直接拷贝（CPU→CPU memcpy，零拷贝或 pin_memory 内拷贝）
        #       到 cpu_k_cache[layer_idx][cpu_block_id] 和 cpu_v_cache 对应位置
        #    d. 记录 block_mapping[(seq_id, layer_name, block_ids[i])] = cpu_block_id
        # 3. 返回 True
        return False

    def load_packed_blocks(
        self,
        layer_name: str,
        seq_id: int,
        block_ids: list[int],
    ) -> tuple[torch.Tensor, torch.Tensor] | None:
        """将 CPU 存储池中的多个 block 拼装为连续 buffer，供 burst DMA 传输回 NPU。

        由 CPUOffloadConnector.ascend_swap_in_with_burst_gather 在
        burst DMA 传输前调用。返回的连续 buffer 可直接用于 npu_burst_copy_to_npu。

        Args:
            layer_name: 层名称
            seq_id:     序列 ID
            block_ids:  需要加载的 NPU 端 block ID 列表

        Returns:
            (k_packed, v_packed): 拼装好的连续 CPU 张量
            若任意 block 不在 CPU 缓存中，返回 None
        """
        # --- 执行逻辑 ---
        # 1. 对每个 block_id in block_ids：
        #    a. 通过 block_mapping 查找对应的 cpu_block_id
        #    b. 若找不到，返回 None
        # 2. 分配连续 CPU buffer：
        #    k_packed = torch.empty([len(block_ids) * block_size, num_kv_heads, head_dim],
        #                           pin_memory=True)
        #    v_packed 同上
        # 3. 对每个 block：
        #    将 cpu_k_cache[layer_idx][cpu_block_id] 拷贝到 k_packed 对应段
        #    将 cpu_v_cache[layer_idx][cpu_block_id] 拷贝到 v_packed 对应段
        # 4. 释放已加载的 cpu_block（调用 free_cpu_block）
